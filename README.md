...
**Example requests** sits under the ask box: 74 questions someone would actually
ask at a bench, grouped by subject, with the groups that fit the loaded sequence
first. Clicking one fills the box rather than sending it, so you can edit before
spending a request. Between them they reach every part of the language —
ambiguity codes, spacing, approximate matching, real base pairing, PROSITE
syntax, and composite elements whose parts are reported separately.
...
web/test/     113 tests, run with `npm test`
