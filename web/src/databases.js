...
/* ------------------------------------------------------- Ensembl species
   ...
   The species field is free text, because Ensembl serves some fifty thousand
   genomes across its divisions and no list of them belongs in a page. What
   the page can do is tell a wrong name from a wrong gene: a lookup that fails
   is followed by one cheap question — does this species exist at all? — so
   "homo_sapein" is reported as a misspelt species and not as a missing gene. */

let speciesList = null;   // the vertebrate list, once asked for

/** Ensembl's vertebrate species, for completion and for spelling suggestions.
    The other divisions — plants, fungi, thirty thousand bacteria — are served
    by the same API but not listed here; a name typed for one of them is
    checked directly instead. Never throws: the list is a convenience. */
export async function ensemblSpecies({ signal } = {}) {
  if (speciesList) return speciesList;
  try {
    const res = await fetch(`${ENSEMBL}/info/species?content-type=application/json`, { signal, headers: { Accept: "application/json" } });
    if (!res.ok) return [];
    const json = await res.json();
    speciesList = (json.species ?? []).map((s) => ({
      name: s.name, display: s.display_name || "", common: s.common_name || "",
      assembly: s.assembly || "", aliases: s.aliases || [],
...
/** Ensembl's own spelling of a species name: lower case, underscores. */
export const speciesKey = (text) => text.trim().toLowerCase().replace(/[\s-]+/g, "_");
...
/** The species the reader probably meant, or null. An exact hit on a common
    name ("human", "mouse") or an alias counts; otherwise the nearest name
    within a few letters, so a slip of the keyboard is caught and a different
    organism is not guessed at. */
export function suggestSpecies(typed, list) {
  const key = speciesKey(typed);
  if (!key || !list?.length) return null;
  const words = key.replace(/_/g, " ");
  for (const s of list) {
    if (s.name === key) return null;   // it is right; nothing to suggest
    const names = [s.display, s.common, ...s.aliases].map((x) => x.toLowerCase());
    if (names.includes(words) || names.includes(key)) return s.name;
  }
  let best = null, bestScore = Infinity;
  for (const s of list) {
    const d = editDistance(key, s.name);
    if (d < bestScore) { best = s.name; bestScore = d; }
  }
  const budget = key.length >= 10 ? 3 : 2;   // a transposition costs two, and a missing letter beside it three
  if (bestScore <= budget) return best;
  const prefixed = list.find((s) => s.name.startsWith(key) && key.length >= 4);
  return prefixed ? prefixed.name : null;
}

/** Whether Ensembl knows a species, and which assembly it serves for it.
    One request, and it works for every division, bacteria included. */
export async function ensemblSpeciesExists(species, { signal } = {}) {
  try {
    const res = await fetch(`${ENSEMBL}/info/assembly/${encodeURIComponent(species)}?content-type=application/json`,
      { signal, headers: { Accept: "application/json" } });
    if (!res.ok) return { ok: false, assembly: "" };
    const json = await res.json();
    return { ok: !json.error, assembly: json.assembly_name || "" };
  } catch (e) {
    if (e.name === "AbortError") throw e;
    return { ok: true, assembly: "" };   // could not tell; blame the gene, not the species
  }
}

/** Run an Ensembl request and, if it fails, find out whether the species was
    the problem before reporting a missing record. */
async function withSpeciesCheck(species, signal, action) {
  try {
    return await action();
  } catch (err) {
    if (!(err instanceof BiomotifError) || !/has no record for/.test(err.message)) throw err;
    const { ok } = await ensemblSpeciesExists(species, { signal });
    if (ok) throw err;
    const guess = suggestSpecies(species, await ensemblSpecies({ signal }));
    throw new BiomotifError(`Ensembl has no species called ${species}.` +
      (guess ? ` Did you mean ${guess}?` : " Names are written like homo_sapiens or mus_musculus."));
  }
}

async function ensemblFetch(query, { signal, species = "homo_sapiens", upstream = 0 }) {
  species = speciesKey(species) || "homo_sapiens";
  ...
  if (region) {
    ...
    const text = await withSpeciesCheck(species, signal,
      async () => (await get(url, { signal, what: query.trim(), where: "Ensembl" })).text());
    const records = guard(parseFasta(text));
  ...
  if (!RE.ensembl.test(id)) {
    info = await withSpeciesCheck(species, signal, () => ensemblLookup(id, species, { signal }));
